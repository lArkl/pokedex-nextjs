    ...
    <MemoryRouter initialEntries={[AppRoutes.PokemonList]}>
      <Routes>
        <Route
          ...
          element={
            <PokemonListFilter onClear={onClearMock} onFilter={onFilterMock} />
          }
        />
      </Routes>
    </MemoryRouter>
...
describe("PokemonListFilter", () => {
  ...
  it("shows input and filter button", () => {
    ...
    expect(
      screen.getByRole("combobox", { name: /types/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("combobox", { name: /abilities/i })
    ).toBeInTheDocument();
  ...
  it("shows defaults values from query params", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryAllByText("Select...")).not.toHaveLength(2)
    );

    expect(screen.getByTestId("list_filter")).toHaveFormValues({
      name: "test",
      types: "1",
    });
  });
